 ...
 * @author Kazwell
 */
public class SaisirNoteÉcritController implements Initializable {

    @FXML
    private AnchorPane WGradeinput;
    ...
    private void makeStageDragable() {
		WGradeinput.setOnMousePressed((event) -> {
			xOffset=event.getSceneX();
			yOffset=event.getSceneY();
		});
		WGradeinput.setOnMouseDragged((event)-> {
			stage = (Stage)((Node)event.getSource()).getScene().getWindow();
			stage.setX(event.getScreenX()-xOffset);
			stage.setY(event.getScreenY()-yOffset);
			stage.setOpacity(0.95f);
		});
		WGradeinput.setOnDragDone((event)-> {
			stage.setOpacity(1.0f);
		});
		WGradeinput.setOnMouseReleased((event)->{
			stage.setOpacity(1.0f);
		});
	}
    ...
    @FXML
    void EnregistrerE_Note(ActionEvent event)throws SQLException {
        MyConnection cnx = new MyConnection();    
        Connection con = cnx.getConnection();
        ...
        try{
        PreparedStatement pst,pstID,pstTest,pstCoeff,pstNote,pstGet,pstSet,pstMoy,pstFin;
        ResultSet rs,rsTest,rsCoeff,rsNote,rsGet,rsMoy;
        ...
        if(!rsTest.next()){
            ...
            Float NoteÉcrit = Float.parseFloat(NoteField.getText());
            int NoteTP = 0;
        ...
        pstNote = con.prepareStatement("SELECT Note_TP FROM étudiant_matière where Nom_Matière = ? AND CNE_Étudiant =? ;");
        pstNote.setString(1,Matiere.getText());
        pstNote.setInt(2,Integer.parseInt(CNEField.getText()));
        rsNote = pstNote.executeQuery();
        
        /*--------------------------------------------------------------------------------------------
        Calculer et afficher les notes de matière
        */
        
        
        Float NoteTP = 0.0f;
        if (rsNote.next()){
            NoteTP = rsNote.getFloat("Note_TP");
        }
        pstCoeff = con.prepareStatement("SELECT Coefficient_Écrit,Coefficient_TP FROM matière WHERE Nom_Matière=? ;");
            pstCoeff.setString(1,Matiere.getText());
            rsCoeff = pstCoeff.executeQuery();
        ...
        Float NoteÉcrit = Float.parseFloat(NoteField.getText());
        pst = con.prepareStatement("UPDATE étudiant_matière SET Note_Écrit=?,Note_Matière =? WHERE CNE_Étudiant=? AND Nom_Matière=?;");
        pst.setFloat(1,NoteÉcrit);
        pst.setInt(3,cne);
...
"    group by étudiant_CNE_Étudiant;");
        ...
        while (rsMoy.next()){
            ...
            //JOptionPane.showMessageDialog(null,rsMoy.getFloat("MoyG"));
